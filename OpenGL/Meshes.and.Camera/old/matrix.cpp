...
Point2D::Point2D(float i, float j){
    x = i;
    y = j;
    h = 1;
}
...
Point2D Matrix::operator*(Point2D& point){
    Point2D r;
    r.x = point.x*m[0] + point.y*m[4] + point.h*m[8];
    r.y = point.x*m[1] + point.y*m[5] + point.h*m[9];
    r.h = point.x*m[2] + point.y*m[6] + point.h*m[10];

    return r;
}

Vector2D Matrix::operator*(Vector2D& vec){
    Vector2D r;

    r.x = vec.x*m[0] + vec.y*m[4] + vec.h*m[8];
    r.y = vec.x*m[1] + vec.y*m[5] + vec.h*m[9];
    r.h = vec.x*m[2] + vec.y*m[6] + vec.h*m[10];
